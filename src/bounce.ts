...
var bouncers: Array<Bouncer> = new Array()
const timerInterval: number = 1500;
const timerId = window.setInterval(() => {
    if (!spawn()) window.clearTimeout(timerId)
}, timerInterval)
...
class Bouncer {
    ...
    private isOutOfBoundX(): boolean {
        return this.x <= 0
            || this.x + this.size >= canvasElement.width
    }

    private isOutOfBoundsY(): boolean {
        return this.y <= 0
    ...
    public draw(): void {
        ...
        if (this.isOutOfBoundX()) this.dx *= -1
        if (this.isOutOfBoundsY()) this.dy *= -1
    }
}

function spawn(ev?: KeyboardEvent): boolean {
    const isFull = bouncers.length >= maxBouncers
    if (!isFull) bouncers.push(new Bouncer())
    return !isFull
}
